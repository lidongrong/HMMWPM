# -*- coding: utf-8 -*-
"""
Created on Tue Mar 14 14:38:46 2023
...
class Random_Gibbs(Optimizer):
    ...
    def joint_core(self,x,y,z,alpha,beta):
        '''
        calculate joint log pdf, used for calculating the gradient wrt x, alpha and beta
        tensor in, tensor out
        y,z are numpy arrays
        ...
        stacked_position=position.flatten()[~np.isnan(position.flatten())]
        stacked_position=stacked_position.astype(int)
        
        means=torch.tensor(self.mu[stacked_position])
        sig=torch.tensor(self.sigma[stacked_position])
        
        # select all xt
        mask=self.model.lengths>np.arange(x.shape[1])[:,None]
        mask=mask.T
        
        # calculate the first term: p(x_t|z_t,mu,sigma)
        log_term1 = torch.bmm((x[mask]-means).unsqueeze(-2),sig)
        log_term1 = log_term1.squeeze(-2)
        log_term1 = -torch.sum(log_term1 * (x[mask]-means))
        f+= log_term1
        
        # turn y to tensor for down stream calculation
        y=torch.tensor(y)
        ...
        for i in range(self.model.hidden_dim):
            for j in range(self.model.hidden_dim):
                ...
                f=f- torch.sum(torch.logsumexp(train_x @ alpha[i].T,dim=1))
    ...
    def abx_grad(self,x,y,z,alpha,beta):
        '''
        take derivative wrt x,alpha and beta
        return obj func, x grad, alpha grad and beta grad
        TEST CODE:
            optimizer.abx_grad(optimizer.x,optimizer.y,optimizer.z,
                               optimizer.alpha,optimizer.beta)
        '''
        alpha=torch.tensor(alpha,requires_grad=True)
        beta=torch.tensor(beta,requires_grad=True)
        x=torch.tensor(x,requires_grad=True)
        
        f=self.joint_core(x,y,z,alpha,beta)
        ...
        x_grad=x.grad
        f=f.detach().numpy()
        return f, x_grad.numpy(),alpha_grad.numpy(),beta_grad.numpy()
    
        
    def MALA_abx(self,x,y,z,alpha,beta):
        '''
        update alpha, gamma and x with MALA
        TEST CODE:
            new_x=optimizer.MALA_abx(optimizer.x,optimizer.y,optimizer.z,
                               optimizer.alpha,optimizer.beta)
        '''
        self.args.hk=1
        lr=self.args.learning_rate * 1.0
        # optimzal learning rate as 1/d
        xlr=1/((np.sum(self.model.lengths)*x.shape[2]-np.sum(self.model.x_masks)))
        
        # update all parameters at one time
        
        # generate noise for different parameters
        #alpha_noise= np.sqrt(2*lr) * np.random.normal(0,1,size=alpha.shape)
        #beta_noise= np.sqrt(2*lr) * np.random.normal(0,1,size=beta.shape)
        x_noise = np.sqrt(2*xlr) * np.random.normal(0,1,size=x.shape)
        ...
        # calculate gradient
        f,x_grad,alpha_grad,beta_grad=self.abx_grad(old_x,y,z,old_alpha,old_beta)
        
        # update parameters
        #new_alpha = old_alpha + lr * alpha_grad +alpha_noise
        #new_beta = old_beta + lr * beta_grad + beta_noise
        new_x = old_x + xlr * x_grad + x_noise
        
        # ensure identifiability
        #new_alpha[:,-1,:]=0
        # ensure observed values unchanged
        new_x[self.model.x_masks]=old_x[self.model.x_masks]
        x_grad[self.model.x_masks]=0
        
        
        f1,x_grad1,alpha_grad1,beta_grad1 = self.abx_grad(new_x,y,z,old_alpha,old_beta)
        
        x_grad1[self.model.x_masks]=0
        
        mask=self.model.lengths>np.arange(x.shape[1])[:,None]
        mask=mask.T
        
        ratio=f1-f - (1/(4*lr))*np.linalg.norm(old_x[mask]-(new_x+xlr*x_grad1)[mask])**2+\
                            (1/(4*lr))*np.linalg.norm(new_x[mask]-(old_x+xlr*x_grad)[mask])**2
        u=np.random.uniform(0,1,1)[0]
        log_u=np.log(u)
        ratio=min(0,ratio)
        if log_u<=ratio:
            #print('accept with rate ',ratio)
            return new_x
        else:
            #print('reject with rate ', ratio)
            return old_x
    ...
    def sample_x(self,x,y,z):
        new_x=self.MALA_abx(x,y,z,self.alpha,self.beta)
        return new_x
    ...
    def xt_forward(self,x,y,z,length,x_masks,y_masks):
        ...
        sig=torch.tensor(self.sigma[position])
        
        # in case sig only contains 1 element
        if len(sig.shape)==2:
          sig=sig.unsqueeze(-3)
        
        log_term1 = torch.bmm((x[:length]-means).unsqueeze(-2),sig)
        log_term1 = log_term1.squeeze(-2)
        log_term1 = -torch.sum(log_term1 * (x[:length]-means))
        f+= log_term1
    ...
    def sys_scan(self,n,log_step=None,prog_bar=True,prob=0.5,initial_x=None,initial_z=None):
        ...
        #for new_ass in range(1):
            for s in tqdm(range(n)):
                ...
                '''
                #start=time.time()
                x=self.sample_x(self.x,self.y,self.z)
                self.x=np.array(x)
                #end=time.time()
                ...
                if not (self.real_x is None):
                    #self.x=x
                    mask=self.model.lengths>np.arange(self.x.shape[1])[:,None]
                    mask=mask.T
                    acc=np.sum((self.x[mask]-self.real_x[mask])**2)
                    acc=acc/np.sum(self.model.x_masks)
                    self.x_acc[s]=acc
                gc.collect()
                #end=time.time()
