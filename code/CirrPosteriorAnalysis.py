...
# posterior path
path='D:/Files/CUHK_Material/Research_MakeThisObservable/EMR/Data/Cirrhosis Data/CirrPosterior/NormalAnalysis2'
# effective numbers
e=4
...
func=np.load(f'{path}/obj_func.npy')
...
post_mu=mu[7000:]
post_beta=beta[7000:]
post_sigma=sigma[7000:]
post_alpha=alpha[7000:]
...
cols=list(TransTable.columns[-1:])+list(TransTable.columns[:-1])
TransTable=TransTable[cols]
t=TransTable.to_markdown()

md=[]
for k in range(4):
    md.append(TransTable.loc[5*k:5*(k+1)-1])
    
for k in range(len(md)):
    print('LINESSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS')
    print(md[k].to_markdown())

# Get Transition coef variance
TransTableVar={k:[] for k in covariates}
TransTableVar['Transition']=[]
for i in range(mean_alpha.shape[0]):
    for j in range(mean_alpha.shape[1]):
        TransTableVar['Transition'].append(f'{stages[i]}->{stages[j]}')
        for k in range(len(covariates)):
            TransTableVar[covariates[k]].append(var_alpha[i][j][k]) 
TransTableVar=pd.DataFrame(TransTableVar)
cols=list(TransTableVar.columns[-1:])+list(TransTableVar.columns[:-1])
TransTableVar=TransTableVar[cols]
t=TransTableVar.to_markdown()

# Get Z scores
TransTableZ={k:[] for k in covariates}
TransTableZ['Transition']=[]
for i in range(mean_alpha.shape[0]):
    for j in range(mean_alpha.shape[1]):
        TransTableZ['Transition'].append(f'{stages[i]}->{stages[j]}')
        for k in range(len(covariates)):
            TransTableZ[covariates[k]].append(2*(1-ss.norm.cdf(
                np.abs(mean_alpha[i][j][k])/np.sqrt(var_alpha[i][j][k])))) 
TransTableZ=pd.DataFrame(TransTableZ)
cols=list(TransTableZ.columns[-1:])+list(TransTableZ.columns[:-1])
TransTableZ=TransTableZ[cols]
t=TransTableZ.to_markdown()
